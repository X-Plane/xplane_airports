...
_class_ `xplane_airports.AptDat.Airport`(_name: str_, _id: str_, _from\_file: str = ''_, _has\_atc: bool = False_, _elevation\_ft\_amsl: float = 0_, _text: List\[[AptDat.AptDatLine](#AptDat.AptDatLine)\]=\[\]_)\
...
- _text_ (List\[[AptDatLine](#AptDat.AptDatLine)\]; default empty): The complete text of the portion of the apt.dat file pertaining to this airport

_static_ `from_lines`(_apt\_dat\_lines_, _from\_file\_name_) -> [Airport](#AptDat.Airport)\
Parameters:\

- **from\_file\_name** (_str_) – The name of the apt.dat file you read this airport in from
- **apt\_dat\_lines** (_collections.Iterable\[[AptDatLine](#AptDat.AptDatLine)|str\]_) – The lines of the apt.dat file (either strings or parsed AptDatLine objects)

_static_ `from_str`(_file\_text_, _from\_file\_name_) -> [Airport](#AptDat.Airport)\
Parameters:
...
Method `runway_type` -> [RunwayType](#AptDat.RunwayType)\
...
_class_ `xplane_airports.gateway.GatewayApt`(_apt: [AptDat.Airport](#AptDat.Airport), txt: Optional\[str\], readme: str, copying: str, pack\_metadata: dict, apt\_metadata: Optional\[dict\]_)

All the data we get back about an airport when we download a scenery pack via `scenery_pack()`

Dataclass members:

- _apt_ ([Airport](#AptDat.Airport)): Python object with the contents of the `apt.dat` file
...
#### `xplane_airports.gateway.recommended_scenery_packs`(_selective\_apt\_ids=None_) -> collections.Iterable\[[GatewayApt](#gateway.GatewayApt)\] 
...
#### `xplane_airports.gateway.scenery_pack`(_pack\_to\_download_) -> [GatewayApt](#gateway.GatewayApt)
